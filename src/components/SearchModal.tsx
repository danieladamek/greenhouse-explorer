...
const KIND_LABEL: Record<SearchHit['kind'], string> = { term: 'Term', concept: 'Concept', compound: 'Compound', plant: 'Plant', preparation: 'Tea Time', figure: 'Figure', section: 'Primer' };

/** ⌘K search over terms, concepts, compounds, plants, preparations, figures and primer sections. */
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-start justify-center bg-black/40 p-4 pt-[10vh]" role="presentation" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div role="dialog" aria-modal="true" aria-label="Search" className="bx-card w-full max-w-xl p-3 bg-paper dark:bg-night" onKeyDown={onKey}>
        <label htmlFor="global-search" className="sr-only">Search terms, concepts, compounds, figures and sections</label>
        <input id="global-search" ref={inputRef} className="bx-input !text-base" placeholder="Search plants, compounds, terms, preparations…" value={q} onChange={(e) => setQ(e.target.value)} autoComplete="off" role="combobox" aria-expanded={hits.length > 0} aria-controls="search-results" aria-activedescendant={hits[sel] ? `hit-${hits[sel].kind}-${hits[sel].id}` : undefined} />
